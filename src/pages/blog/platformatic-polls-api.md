...
Platformatic DB solves the problem of having to create and endpoint that solely returns data from a database. With Platformatic DB, you don’t need to write those endpoints anymore as Platformatic DB will map your Database to REST/GraphQL endpoints to expose to your API consumers.
...
And of course since are using Platformatic DB, we will be able to expose both REST and GraphQL endpoints with minimal work.
...
To create a new Platformatic DB project,
...
Fortunately, Platformatic DB let us create custom mutations and we already reated the file that we can write this custom mutation in - `plugin.js`. So let's write it.
...
In the same `plugin.js` file, let's set up a custom endpoint to create a poll with options. To do so add the following code below the line you called `app.graphql.defineResolvers()`
...
Let start with an async function called `vote`. Edit `plugin.js` to have a the below function
...
Let's create a mutation type called `vote`:
